...
const Testimonial = ({ handleMouseMove, spot }) => 
{
    ...
    return (
        <section id="testimonial"className="relative min-h-screen flex items-center justify-between bg-linear-to-br from-green-100 via-emerald-200 to-green-300 text-green-900 overflow-hidden px-2 sm:px-6 pt-18 md:pt-14" onMouseMove={handleMouseMove}>
            {/* Noise */}
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mt-4">
                    {
                        testimonials.map((testimonial, index) => (
                            <motion.div  key={index} initial={{ opacity: 0, x: index % 2 === 0 ? -40 : 40 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 + index * 0.1 }} viewport={{ once: false }} className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-green-200 hover:shadow-2xl hover:shadow-green-500/20 transition-all duration-300 mb-3 md:mb-0">
